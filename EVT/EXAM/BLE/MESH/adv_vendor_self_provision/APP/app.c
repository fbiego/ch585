...
const uint16_t vendor_sub_addr = 0xC001;        // Configure the subscription group address of the custom model
...
void keyPress(uint8_t keys)
{
    ...
    switch(keys)
    {
        default:
        {
            // Send data to the subscription address
            if(1)
            {
                int status;
            ...
            // Send a command to delete nodes and delete it through the application layer custom protocol. Here, it is assumed that the node with the address of 0x0002 is deleted.
            if(0)
            {
                int status;
