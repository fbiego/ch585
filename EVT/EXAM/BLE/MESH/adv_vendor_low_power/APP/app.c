...
static void link_close(bt_mesh_prov_bearer_t bearer, uint8_t reason)
{
    ...
    // After delaying an event, the LPN is enabled. In order to wait for the matching network user routine to issue a command to bind the appkey
 ...
 *
 * @param frd_addr - Friends node address
...
void blemesh_on_sync(void)
{
    ...
    if(bt_mesh_is_provisioned())
    {
        ...
        // If you reply to network distribution information from flash, you will directly enter LPN mode.
...
static uint16_t App_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if(events & APP_LPN_ENABLE_EVT)
    {
        ...
        {
            // After distributing the network, start lpn and start sending a friend request
#if(CONFIG_BLE_MESH_LOW_POWER)
