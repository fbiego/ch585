...
uint16_t gen_lightness_srv_keys[CONFIG_MESH_MOD_KEY_COUNT_DEF] = {BLE_MESH_KEY_UNUSED};             // Add to define the brightness attribute
uint16_t gen_lightness_srv_groups[CONFIG_MESH_MOD_GROUP_COUNT_DEF] = {BLE_MESH_ADDR_UNASSIGNED};

uint16_t gen_color_srv_keys[CONFIG_MESH_MOD_KEY_COUNT_DEF] = {BLE_MESH_KEY_UNUSED};             // Define color temperature properties
...
static struct bt_mesh_model root_models[] = {
    ...
    BLE_MESH_MODEL(BLE_MESH_MODEL_ID_LIGHT_LIGHTNESS_SRV, gen_lightness_op, NULL, gen_lightness_srv_keys, gen_lightness_srv_groups, NULL),      // Add root brightness attribute
    BLE_MESH_MODEL(BLE_MESH_MODEL_ID_LIGHT_CTL_TEMP_SRV , gen_color_op, NULL, gen_color_srv_keys, gen_color_srv_groups, NULL),      // Add root color temperature attribute
};
...
static void link_close(bt_mesh_prov_bearer_t bearer, uint8_t reason)
{
    ...
    else
    {
        ...
        /* For Light Subscription group address */          // Added Brightness Attribute
        ...
        /* For Light Subscription group address */          // Added color temperature attribute
...
void send_support_attr(void)
{
    ...
    // // Here you add the brightness attribute for reference, and you also need to add the corresponding opcode processing function. The method refers to the gen_onoff_op structure of the switch attribute.

    // Add brightness attributes
    {
        /* Add brightness attrbute opcode */
        net_buf_simple_add_le16(&(ind->buf->b), ALI_GEN_ATTR_TYPE_BRIGHTNESS);

    /* Add brightness status (655~65535 corresponds to Tmall control brightness 1~100) */
        net_buf_simple_add_le16(&(ind->buf->b), 65535);
    }

    // Add color temperature attributes
    {
        /* Add brightness attrbute opcode */
        net_buf_simple_add_le16(&(ind->buf->b), ALI_GEN_ATTR_TYPE_COLOR);

    /* Add brightness status (992~20000 corresponds to Tmall color temperature 1~100) */
...
void send_led_state(void)
{
    ...
    // toggle_led_state(MSG_PIN); //The state of the flip light
    ...
    send_led_indicate(&param);                  // Switch status of the sending lamp
    send_lightness_indicate(&param);            // Brightness status of the sending lamp
    send_color_indicate(&param);                // Send light color temperature status
}
 ...
 *
 * @return none */
#if 0       // During debugging, call to view the unknown callback status
...
void App_Init(void)
{
    ...
    // bt_mesh_model_reg_elem_unkonw_op_cb((elem_unkonw_op_cb_t)my_test1); //Initialize the call to check the unknown callback status when the Tmall command is sent
}
